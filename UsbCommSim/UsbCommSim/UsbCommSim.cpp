// UsbCommSim.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; devs[i]; i++)
	{
		...
		dev_ptr = devs[i];
		r = libusb_get_device_descriptor(dev_ptr, &desc);
		if (r < 0)
		{
		...
		if ((desc.idVendor == 0x153A) && (desc.idProduct == 0x0030))
		{
			dev_cur = dev_ptr;
			break;
		}
	...
	r = libusb_open(dev_cur, &dev_handle);
	if (dev_handle == NULL)
	{
	...
	libusb_get_configuration(dev_handle, &config_num);
	libusb_get_config_descriptor(dev_cur, 0, &config_desc);
	if (config_desc)
	{
		unsigned char data_Tx[SIZE_OUT_DATA_1];
		unsigned char data_Rx[64];
		int length = 0, actLen = 0;
		unsigned char ep_in_addr, ep_out_addr;
